...
module.exports = function(app, mysql_connection) {
    ...
    app.get('/email/send-verification/:email(*)', (req, res) => {
        ...
        const options = {
            ...
            // text: `Please click this link to verify your account: http://3.218.225.62:3040/email/verify/${email_code}`,
            html: `
                <div>
                    <span>
                        Please click this link to verify your account: 
                        <a href="http://3.218.225.62:3040/email/verify/${email_code}">Verify Now</a>
                    </span>
                </div>
            `
        };
            ...
            html: `
                <div>
                    <span>
                        ...
                        <a href="http://3.218.225.62:3040/email/verify-password-reset/${email_code}">Reset Now</a>
                    </span>
                </div>
            `
        };
    ...
    app.get('/email/verify/:code(*)', (req, res) => {
        ...
        mysql_connection.query(`UPDATE USER SET User_status = 1 WHERE User_email = '`+decrypted_email+`';`, function (err, rows, fields) {
            if (err) {
                ...
                res.redirect('http://44.199.110.21:3000/login');
            }
        });
    });
    ...
    app.get('/email/verify-password-reset/:code(*)', (req, res) => {
        ...
        mysql_connection.query(`SELECT User_id FROM USER WHERE User_email = '`+decrypted_email+`';`, function (err, rows, fields) {
            if (err) {
                ...
                res.redirect('http://44.199.110.21:3000/password-reset/'+rows[0].User_id);
            }
        });
    });
